...
import type { FontWeight } from "./render/types.ts";
...
export function measureCtx(regularPath: string, boldPath: string): MeasureCtx {
...
function unitWidth(ctx: MeasureCtx, text: string, weight: FontWeight): number {
...
export function fitSizePt(
  ...
  heightPt: number,
  startPt: number,
  opts: { weight?: FontWeight; floorPt?: number; stepPt?: number } = {},
): number | null {
  const weight = opts.weight ?? 400;
  const floor = opts.floorPt ?? 12;
  const step = opts.stepPt ?? 1;
  for (let size = startPt; size >= floor; size -= step) {
    if (measureHeightPt(ctx, text, widthPt, size, weight) <= heightPt) return size;
  }
  return null;
}
