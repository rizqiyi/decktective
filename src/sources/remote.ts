...
export type ResolvedRepo = {
  /** Absolute local path to the work tree. */
  path: string;
  /** Stable id used as the source id on every DayEntry. */
  id: string;
  /** What the caller pointed at, for logging. */
  source: string;
  /** True when this call created the local clone. */
  cloned: boolean;
  /** True when an existing clone was refreshed. */
  fetched: boolean;
};
...
const gitOk = async (dir: string): Promise<boolean> => {
  try {
...
export type ResolveOptions = {
  /** Where clones live. Defaults to the decktective cache dir. */
  cacheDir?: string;
  /** Use an existing clone without contacting the remote. */
  offline?: boolean;
};
...
export async function resolveRepo(
  ...
  if (raw === "") throw new RepoError("no repository given");
  ...
  if (!looksRemote) {
    if (!localExists) throw new RepoError(`no such directory: ${asLocal}`);
  ...
  if (await pathExists(join(dir, ".git"))) {
    ...
    try {
      ...
      throw new RepoError(
        `could not refresh ${url}: ${err instanceof Error ? err.message : String(err)}. ` +
