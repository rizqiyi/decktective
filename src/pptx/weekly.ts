...
import {
  fillSlide, removeShapesById, readShapes, setShapeText, cloneShapes, setShapeBox,
  type Bindings, type ShapeInfo,
} from "./fill.ts";
import { plural } from "../narrative/template.ts";
import type { DayEntry, ItemKind, Narrative, Window, WorkItem } from "../ir/types.ts";
...
function itemsForDay(workItems: WorkItem[], date: string, n: number): WorkItem[] {
  return workItems
...
function rangeLabel(window: Window): string {
...
function selectDays(
  days: DayEntry[],
  max = MAX_DAYS,
): { shown: DayEntry[]; dropped: DayEntry[] } {
  const active = days.filter((d) => d.metrics.commits > 0);
  if (active.length <= max) return { shown: active, dropped: [] };
  const ranked = [...active].sort(
    (a, b) => b.metrics.commits - a.metrics.commits || a.date.localeCompare(b.date),
  );
...
function coverBindings(window: Window): Bindings {
  return { "[Sep 7 \u2013 Sep 11]": rangeLabel(window) };
}

function overviewBindings(
  days: DayEntry[],
  narrative: Narrative,
  workItems: WorkItem[],
): Bindings {
  ...
  return {
    ...
    "[27]": String(commits),
    "[Main feature / delivery completed or progressed]":
      clamp(top[0]?.title ?? themes[0] ?? "", NARRATIVE_MAX),
    "[Important bug / stability improvement]":
      clamp(top[1]?.title ?? themes[1] ?? "", NARRATIVE_MAX),
    "[Cross-repo or technical work that had meaningful impact]":
      clamp(top[2]?.title ?? themes[2] ?? "", NARRATIVE_MAX),
  };
}
 ...
 */
function dayBindings(day: DayEntry, index: number): Bindings {
  ...
  return {
    ...
    "[+420 / -110]":
      ` \u00b7 +${day.metrics.additions.toLocaleString("en-US")} / -${day.metrics.deletions.toLocaleString("en-US")}`,
...
function highlightsBindings(workItems: WorkItem[]): Bindings {
  const top = [...workItems]
    .sort((a, b) => b.additions + b.deletions - (a.additions + a.deletions))
    .slice(0, 3);
  const note = (w: WorkItem | undefined): string =>
    w ? `${KIND_LABEL[w.type]} \u00b7 ${w.impact} \u00b7 ${plural(w.refs.length, "commit")} \u00b7 ${plural(w.files, "file")}` : "";
  return {
...
const SUMMARY_COLUMNS = ["title", "repos", "type", "commits", "impact", "status"] as const;

type SummaryRow = { panelIds: string[]; cellIds: string[]; y: number };
 ...
 */
function readSummaryRows(shapes: ShapeInfo[]): SummaryRow[] {
  const bands = new Map<number, ShapeInfo[]>();
  for (const s of shapes) {
    const key = Math.round(s.y * 100) / 100;
    bands.set(key, [...(bands.get(key) ?? []), s]);
  }
  const bandsOut: SummaryRow[] = [];
  for (const y of [...bands.keys()].sort((a, b) => a - b)) {
    const band = (bands.get(y) ?? []).sort((a, b) => a.x - b.x);
    if (band.length !== SUMMARY_COLUMNS.length) continue;
    if (band.every((s) => s.text.trim() === "")) continue; // a panel band
...
function fillSummaryTable(
  xml: string,
  workItems: WorkItem[],
): { xml: string; rows: number; truncated: number; missing: string[] } {
  ...
  for (let c = 0; c < SUMMARY_COLUMNS.length; c++) {
    ...
    if (panel !== undefined) sourceIds.push(panel);
    if (cell !== undefined) sourceIds.push(cell);
  }
  ...
  for (let r = 0; r < sorted.length; r++) {
    const item = sorted[r]!;
    const row = rows[r]!;
    const values: Record<(typeof SUMMARY_COLUMNS)[number], string> = {
    ...
    SUMMARY_COLUMNS.forEach((col, c) => {
      ...
      if (id !== undefined) out = setShapeText(out, id, values[col]);
    });
  }
...
function closingBindings(narrative: Narrative): Bindings {
...
const CARD_PANEL = { x: 0.49, w: 6.19 } as const;
/** Text column, kept clear of the impact badge at x 5.44. */
const CARD_TEXT = { x: 0.71, w: 4.55 } as const;
const CARD_BADGE = { x: 5.44, w: 0.94, h: 0.34, textX: 5.57, textW: 0.68, textDy: 0.09 } as const;
...
/** Shape ids of one card, in `CARD.shapes` order. */
type CardIds = {
  panel: string;
  title: string;
  type: string;
...
function cardIds(ids: readonly string[]): CardIds | undefined {
  const [panel, title, type, detail, badge, badgeText] = ids;
  if (
    panel === undefined || title === undefined || type === undefined ||
...
function layoutDayCards(
  ...
  if (first === undefined) return { xml, missing: [...CARD.shapes] };
  ...
  const missing: string[] = [...drop.missing];
  let out = drop.xml;
  ...
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    const card = cards[i];
    if (item === undefined || card === undefined) break;
...
const CLOSING_TITLE = { from: "Blockers & Next Week", to: "Blockers & Risks" } as const;
 ...
 */
const BLOCKERS_BOXES: Array<{ id: string; x?: number; y?: number; w?: number; h?: number }> = [
...
export type FillOptions = {
  ...
  days: DayEntry[];
  narrative: Narrative;
  /** Grouped work items — the template is built around these, not commits. */
  workItems: WorkItem[];
  window: Window;
  maxDays?: number;
};
...
export type FillResult = {
  dayCount: number;
  droppedDays: string[];
  /** Placeholders the template still shows with no value available from git. */
  unfilled: string[];
  filledCount: number;
};

export async function fillWeeklyTemplate(opts: FillOptions): Promise<FillResult> {
  const pkg = await PptxPackage.load(opts.templatePath);
  const slides = pkg.slides();
  if (slides.length !== SHAPE.slideCount) {
    throw new Error(
      `${opts.templatePath}: expected ${SHAPE.slideCount} slides, found ${slides.length}; ` +
  ...
  for (let i = 1; i < shown.length; i++) lastDay = await pkg.duplicate(lastDay);

  const layout = pkg.slides();
  const unfilled = new Set<string>();
  let filledCount = 0;

  const apply = async (part: string, bindings: Bindings): Promise<void> => {
    const report = fillSlide(await pkg.xml(part), bindings);
    pkg.setXml(part, report.xml);
    filledCount += report.filled.length;
    for (const u of report.unfilled) unfilled.add(u);
  };

  /** Drop a block the week has no content for, rather than leaving it empty. */
  const drop = async (part: string, ids: readonly string[], label: string): Promise<void> => {
    const report = removeShapesById(await pkg.xml(part), ids);
    pkg.setXml(part, report.xml);
    if (report.missing.length > 0) {
      console.log(
        `  warn: ${label}: ${report.missing.length} shape id(s) not found ` +
  ...
  for (let i = 0; i < shown.length; i++) {
    ...
    if (!ref) throw new Error(`missing day slide ${i + 1} after cloning`);
    const day = shown[i]!;
    ...
    const cards = layoutDayCards(
      await pkg.xml(ref.part),
      itemsForDay(opts.workItems, day.date, MAX_DAY_CARDS),
    );
    pkg.setXml(ref.part, cards.xml);
    if (cards.missing.length > 0) {
      console.log(
        `  warn: day ${day.date}: ${cards.missing.length} card shape id(s) not found ` +
  ...
  const summaryPart = layout[tail + 1]!.part;
  {
    ...
    for (const m of report.missing) console.log(`  warn: summary table: ${m}`);
  }
  ...
  // Reclaim the space that block occupied.
  {
    ...
    for (const box of BLOCKERS_BOXES) xml = setShapeBox(xml, box.id, box);
