...
import { resolveRepo } from "./sources/remote.ts";
...
import { loadTemplate, DEFAULT_TEMPLATE_PATH } from "./template/load.ts";
import { fillWeeklyTemplate, KNOWN_GAPS } from "./pptx/weekly.ts";
import { outlineTemplate } from "./pptx/outline.ts";
import { executeManifest, type DeckData } from "./pptx/execute.ts";
import { isoWeekLabel } from "./deck/build.ts";
import {
  learnManifest, loadManifest, saveManifest, manifestCachePath,
} from "./pptx/manifest.ts";
import { resolveProvider, isLive, OfflineProvider, type LlmProvider } from "./llm/provider.ts";
import { runPipeline } from "./work/pipeline.ts";
import {
  loadStyleContract, installStyleSkill, STYLE_SKILL_NAME, type StyleContract,
} from "./style/contract.ts";
import { outlineToPrompt } from "./pptx/outline.ts";
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (k) {
      ...
      case "--pptx-template": a.pptxTemplate = v; i++; break;
      case "--max-days": a.maxDays = Number(v); i++; break;
      case "--llm": a.llm = v; i++; break;
      case "--offline": a.offline = true; break;
      case "--attempts": a.attempts = Number(v); i++; break;
      case "--style": a.style = v; i++; break;
      case "--install-style": a.installStyle = true; break;
...
function makeTrace(enabled: boolean): (message: string) => void {
 ...
 */
function tracingProvider(inner: LlmProvider, trace: (m: string) => void): LlmProvider {
  return {
    id: inner.id,
    model: inner.model,
    async complete(req) {
...
async function main(): Promise<void> {
  ...
  if (args.templateOutline) {
    ...
    if (target === undefined) throw new Error("--template-outline needs --pptx-template <file>");
  ...
  if (args.learnTemplate) {
    const target = args.pptxTemplate;
    if (target === undefined) throw new Error("--learn-template needs --pptx-template <file>");
  ...
  if (repo.cloned) console.log(`Cloned ${repo.source} -> ${repo.path}`);
  else if (repo.fetched) console.log(`Refreshed ${repo.path}`);
  ...
  const baseProvider = args.offline ? new OfflineProvider() : resolveProvider(
    args.llm === undefined ? {} : { spec: args.llm });
  ...
  let contract: StyleContract | undefined;
  if (isLive(provider)) {
  ...
  const pipeline = await stage(trace, "pipeline (group \u2192 articulate \u2192 review)", () =>
    runPipeline(days, {
      provider,
      window,
  ...
  console.log(
    `Grouped ${days.reduce((a, d) => a + d.items.length, 0)} item(s) into ` +
  ...
  if (pipeline.attempts > 1) console.log(`  review loop: ${pipeline.attempts} attempt(s)`);
  for (const h of pipeline.history) {
    ...
    for (const v of h.violations) trace(`    [${v.rule}] ${v.where}: ${v.detail.slice(0, 90)}`);
  }
  ...
  if (pipeline.violations.length > 0) {
    ...
    for (const v of pipeline.violations) console.log(`    [${v.rule}] ${v.where}: ${v.detail}`);
  }
  ...
  if (args.pptxTemplate) {
    ...
    if (args.manifest) {
      ...
      if (result.unsupported.length > 0) {
        ...
        for (const u of [...new Set(result.unsupported)]) console.log(`    ${u}`);
      }
      return;
    }
    ...
    if (result.unfilled.length > 0) {
      ...
      for (const u of result.unfilled) console.log(`    ${u}`);
    }
