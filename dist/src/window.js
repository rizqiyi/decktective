...
export function isoInZone(tz, at) {
...
export function instantInZone(tz, y, m, d, h = 0, min = 0, s = 0) {
...
export function dateInZone(tz, at) {
...
export function isoWeekWindow(tz, at) {
...
export function previousIsoWeek(tz, at) {
...
export function lastNWeeks(tz, at, n) {
    const cur = isoWeekWindow(tz, at);
...
export function presets(tz, now = new Date()) {
    return [
 ...
 */
export function densityHistogram(counts, weeks, width = 28) {
...
export function describeWindow(w) {
    ...
    return (`${fmt.format(new Date(w.start))}  \u2013  ${fmt.format(new Date(w.end))}  ` +
        `(${w.tz}, half-open)`);
}
