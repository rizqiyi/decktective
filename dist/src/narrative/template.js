...
function headlineFor(day) {
    if (day.items.length === 0)
        return "No recorded activity";
    const top = [...day.items].sort((a, b) => (b.additions ?? 0) + (b.deletions ?? 0) - ((a.additions ?? 0) + (a.deletions ?? 0)))[0];
...
function risksFor(days) {
    ...
    if (quiet.length > 0)
        out.push(`${plural(quiet.length, "day")} with no recorded activity.`);
    return out;
}
function summaryFor(days, window) {
    ...
    return (`${commits.toLocaleString("en-US")} commits across ${active} active day(s): ` +
...
function nextWeekFor(days) {
    const out = [];
    const reverts = days.flatMap((d) => d.items).filter((i) => i.kind === "revert");
    if (reverts.length > 0)
        out.push(`Revisit ${reverts.length} reverted change(s).`);
    const chores = days.flatMap((d) => d.items).filter((i) => i.kind === "chore");
    if (chores.length > 0)
        out.push(`Follow up on ${chores.length} chore commit(s).`);
    return out;
}
export class TemplateNarrative {
    async narrate(facts, window) {
