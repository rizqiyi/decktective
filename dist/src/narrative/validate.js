export class InventedNumberError extends Error {
    found;
    constructor(found) {
        super(`narrative contains ${found.length} number(s) not present in the facts: ` +
...
export function allowedNumbers(facts) {
    ...
    for (const d of facts) {
        ...
        add(m.filesTouched);
        add(m.grossChurn);
        add(Math.abs(m.netChurn));
        add(m.concentration);
        add(m.medianCommitSize);
        add(m.maxCommitShare);
        add(d.items.length);
        for (const item of d.items) {
            add(item.additions ?? 0);
            add(item.deletions ?? 0);
            add(item.files ?? 0);
    ...
    if (cGross > 0)
        add(Math.round((cAdd / cGross) * 100));
    if (facts.length > 0)
        add(Math.round(cCommits / facts.length));
    return out;
}
/**
 ...
 */
export function assertFactLocked(narrative, facts) {
    ...
    if (found.length > 0)
        throw new InventedNumberError([...new Set(found)]);
}
