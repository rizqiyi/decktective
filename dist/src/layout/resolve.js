...
import { measureCtx, fitSizePt, wrapLines, lineHeightPt, missingGlyphs, } from "../measure.js";
import { cellRect, roleSizePt } from "../theme.js";
...
export class MissingGlyphError extends Error {
    characters;
    constructor(characters, where, text) {
...
function assertDrawable(ctx, text, where) {
    const missing = missingGlyphs(ctx, text);
    if (missing.length > 0)
        throw new MissingGlyphError(missing, where, text);
}
/** Resolve one text block into a fitted TextOp. */
function textOps(ctx, rectIn, text, role, t, where) {
    ...
    if (fit === null) {
        ...
        throw new OverflowError(`${where}: text does not fit its slot even at the minimum size.\n` +
...
function bulletsHeightPt(ctx, items, widthPt, sizePt, gapPt) {
    let total = 0;
...
function bulletOps(ctx, rectIn, items, role, t, where) {
    for (const item of items)
        assertDrawable(ctx, item, where);
    ...
    for (; size > floor; size -= 1) {
        if (bulletsHeightPt(ctx, items, widthPt, size, gapPt) <= heightPt)
            break;
    }
    const total = bulletsHeightPt(ctx, items, widthPt, size, gapPt);
    if (total > heightPt) {
        throw new OverflowError(`${where}: bullets overflow their slot even at ${size}pt (${total.toFixed(0)}pt > ${heightPt.toFixed(0)}pt). ` +
...
function blockOps(ctx, block, rectIn, role, t, where) {
    switch (block.kind) {
        case "text":
        ...
        case "metrics": {
            ...
            block.items.forEach((m, i) => {
                ...
                const valueFit = fitSizePt(ctx, m.value, inToPt(colW), inToPt(valueRect.hIn), valueSize, { weight: 700, floorPt: 14 });
                if (valueFit === null) {
        ...
        case "evidence": {
            ...
            lines.forEach((line, i) => {
                ...
                if (y + lineH / 72 > rectIn.yIn + rectIn.hIn)
                    return;
                ops.push({
                    op: "text",
...
function slidePlan(ctx, slide, spec, t, index) {
    const ops = [];
    ...
    for (const slot of spec.slots) {
        ...
        if (!slot.accepts.includes(block.kind)) {
            throw new OverflowError(`${where}: layout does not accept block kind "${block.kind}" (accepts: ${slot.accepts.join(", ")})`);
        }
...
export function resolvePlan(ir) {
    ...
    return {
        ...
        slides: ir.slides.map((s, i) => {
            ...
            if (!spec) {
                throw new OverflowError(`slide ${i + 1}: template has no layout "${s.layout}" (has: ${[...byId.keys()].join(", ")})`);
            }
