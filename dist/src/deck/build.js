...
const textBlock = (t, emphasis) => ({ kind: "text", text: t, emphasis });
const bulletBlock = (items, refs) => ({
    kind: "bullets",
    items: items.map((t, i) => ({ text: t, refs: refs?.[i] })),
});
/** A bullet is one line the reader can absorb. */
function clampBullet(text, maxWords = MAX_BULLET_WORDS) {
...
export function isoWeekLabel(startIso, tz) {
...
function resolveFill(fill, ctx) {
    ...
    switch (fill.kind) {
        ...
        case "window-label":
            return textBlock(`${formatDate(ctx.window.start, ctx.window.tz)} \u2013 ` +
                `${formatDate(ctx.window.end, ctx.window.tz)}  \u00b7  ` +
                `${ctx.window.tz}  \u00b7  ${ctx.week}`, "muted");
        case "summary":
        ...
        case "themes":
            return bulletBlock(n.themes.slice(0, MAX_BULLETS).map((th) => `${th.title} (${th.refs.length})`));
        case "risks":
        ...
        case "evidence": {
            const rows = days.flatMap((d) => d.items.flatMap((i) => (i.refs ?? []).map((r) => ({
...
function stepRepetitions(step, ctx) {
    if (step.repeat === "per-day")
        return ctx.narrative.days.length;
    const evidence = Object.values(step.fill).find((f) => f.kind === "evidence");
    if (!evidence)
        return 1;
    const rows = ctx.days.reduce((n, d) => n + d.items.reduce((m, i) => m + (i.refs?.length ?? 0), 0), 0);
    return Math.max(1, Math.ceil(rows / EVIDENCE_PER_PAGE));
}
function stepSlides(step, base) {
    const slides = [];
    ...
    for (let rep = 0; rep < reps; rep++) {
        ...
        if (Object.keys(slots).length === 0)
            continue;
        const slide = { layout: step.layout, slots };
        if (step.title !== undefined)
            slide.title = interpolate(step.title, ctx);
        const day = ctx.day;
        if (day) {
...
export function buildDeck(days, narrative, window, template, opts = {}) {
