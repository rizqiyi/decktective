 ...
 */
export const INSTALLED_SKILL_PATH = join(PACKAGE_ROOT, "skills", STYLE_SKILL_NAME, "SKILL.md");
export const UPSTREAM_SKILL_URL = `https://raw.githubusercontent.com/ayghri/${STYLE_SKILL_NAME}/main/skills/${STYLE_SKILL_NAME}/SKILL.md`;
...
async function fetchText(url) {
    ...
    if (!res.ok)
        throw new Error(`cannot fetch style contract from ${url}: HTTP ${res.status}`);
    return res.text();
}
export async function loadStyleContract(opts = {}) {
    ...
    if (opts.offline === true) {
        throw new Error("no style contract available offline: install it with `node src/cli.ts --install-style`, " +
            "or pass --style <path>");
    }
...
export async function installStyleSkill() {
