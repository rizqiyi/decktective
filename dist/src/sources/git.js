 ...
 */
import { classify } from "./profile.js";
...
export class GitSource {
    ...
    async collect(window, opts) {
        const exclude = opts?.exclude ?? [];
        const branch = opts?.branch;
        if (branch !== undefined)
            await this.assertRef(branch);
        ...
        const [nameOut, churnOut] = await Promise.all([
            this.git(["log", ...flags, "--date=iso-strict-local", NAME_FORMAT, ...revision], window.tz),
            commitsOnly
                ? Promise.resolve(undefined)
                : this.git(["log", ...churnFlags, "--numstat", CHURN_FORMAT, ...revision], window.tz),
        ]);
        const commits = parseCommits(nameOut);
        const churnBySha = churnOut === undefined ? undefined : parseChurn(churnOut, exclude);
        const days = new Map();
        for (const commit of commits) {
            ...
            if (size > agg.maxCommitSize)
                agg.maxCommitSize = size;
            if (churn !== undefined) {
                agg.additions += churn.additions;
                agg.deletions += churn.deletions;
                for (const path of churn.paths)
                    agg.files.add(path);
                for (const prefix of churn.excludedPrefixes)
                    agg.excludedPrefixes.add(prefix);
            }
            if (commit.authorDate.slice(0, 10) !== commit.committerDate.slice(0, 10)) {
                agg.warnings.push(`commit ${commit.sha.slice(0, 7)} by ${commit.author}: author date ${commit.authorDate} and committer date ${commit.committerDate} fall on different days`);
            }
        }
        const entries = [];
        for (const agg of days.values()) {
            ...
            if (agg.excludedPrefixes.size > 0) {
                warnings.push(`excluded paths matching: ${[...agg.excludedPrefixes].sort().join(", ")}`);
            }
            const items = agg.commits.map((commit) => {
                const churn = churnBySha?.get(commit.sha);
                ...
                if (commit.body !== "")
                    item.body = cleanBody(commit.body);
                return item;
            });
            const entry = {
                date: agg.date,
            ...
            if (churnBySha !== undefined)
                entry.profile = classify(metrics);
    ...
    async git(args, tz) {
        return new Promise((resolve, reject) => {
            execFile("git", args, {
                ...
                if (error !== null) {
                    reject(new Error(`git ${args[0] ?? "log"} failed in ${this.path}: ${stderr.trim() || error.message}`));
                    return;
                }
                resolve(stdout);
            });
    ...
    async assertRef(ref) {
        ...
        throw new Error(`--branch ${JSON.stringify(ref)} is not a branch, tag or ref in this repository` +
            (available === "" ? "" : `\n  available: ${available}`));
    }
    async isShallow() {
        try {
 ...
 */
function parseChurn(out, exclude) {
    const bySha = new Map();
    let current;
...
function matchingPrefix(paths, prefixes) {
