...
import { readShapes, setShapeTextFitted } from "./fill.js";
import { plural } from "../narrative/template.js";
const JOIN = "  \u00b7  ";
const shortDate = (iso, tz) => new Intl.DateTimeFormat("en-GB", { timeZone: tz, day: "numeric", month: "short", year: "numeric" })
    .format(new Date(iso));
function windowLabel(w, week) {
...
const totals = (days) => days.reduce((a, d) => ({
...
const itemsForDay = (items, date) => items
 ...
 */
export function renderFill(kind, data, day) {
 ...
 */
export async function executeManifest(opts) {
    ...
    const clonesByDay = new Map();
    if (perDayStep !== undefined) {
        const refs = pkg.slides();
        const proto = refs[perDayStep.slide - 1];
        if (proto === undefined)
            throw new Error(`manifest names slide ${perDayStep.slide}, which does not exist`);
        let last = proto.part;
    ...
    const partForStep = (slide, dayIndex) => {
        const perDayAt = perDayStep === undefined ? -1 : perDayStep.slide - 1;
        if (perDayAt >= 0 && slide - 1 === perDayAt)
            return clonesByDay.get(dayIndex);
