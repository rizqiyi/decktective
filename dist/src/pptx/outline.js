 ...
 */
export async function outlineTemplate(path, opts = {}) {
    ...
    if (sz)
        canvas = { w: round2(Number(sz[1]) / 914400), h: round2(Number(sz[2]) / 914400) };
    for (const [i, ref] of refs.entries()) {
        const shapes = readShapes(await pkg.xml(ref.part));
        const out = [];
...
export function outlineToPrompt(outline) {
    const lines = [
    ...
    for (const s of outline.slides) {
        ...
        for (const sh of s.shapes) {
            ...
            lines.push(`  id=${sh.id} x=${sh.x} y=${sh.y} w=${sh.w} h=${sh.h}`
