...
function impactOf(additions, deletions) {
 ...
 */
export function groupDeterministic(days) {
    const drafts = [];
    for (const day of days) {
        ...
        for (const [kind, items] of byKind) {
            const heaviest = [...items].sort((a, b) => (b.additions ?? 0) + (b.deletions ?? 0) - ((a.additions ?? 0) + (a.deletions ?? 0)))[0];
            if (!heaviest)
                continue;
...
const GROUP_SYSTEM = "You group a week of git commits into coherent WORK ITEMS — the pieces of work " +
...
function groupPrompt(facts, window) {
...
export function parseDrafts(raw) {
    ...
    for (const entry of items) {
        ...
        if (!Array.isArray(d.refs) || d.refs.length === 0)
            continue;
        const refs = d.refs.filter((r) => typeof r === "string" && r !== "");
        if (refs.length === 0)
            continue;
        const type = typeof d.type === "string" && KINDS.includes(d.type)
            ? d.type
            : "other";
        const status = typeof d.status === "string" && STATUSES.includes(d.status)
            ? d.status
            : undefined;
        out.push({
    ...
    if (out.length === 0)
        throw new Error("no usable work items in response");
    return out;
}
/**
 ...
 */
export function materialize(drafts, facts) {
    ...
    if (unassigned.length > 0) {
        ...
        items.push(buildItem({ title: "Other changes", type: "other", refs: unassigned }, unassigned, bySha));
    }
    return { items, unassigned, fabricated };
}
function buildItem(draft, refs, bySha) {
    let additions = 0;
    let deletions = 0;
    let files = 0;
    const days = new Set();
...
export async function groupWorkItems(days, opts) {
    const facts = factsOf(days);
    if (facts.length === 0)
        return { items: [], unassigned: [], fabricated: [], usedModel: false };
